...
```mermaid
flowchart TD
    A[Scan Start Request] --> B[Data Collection and Preprocessing]
    B --> C[Generate Scan Point Candidates]
    C --> D[Input Candidates into KNN Model]
    D --> E{Meets Nearest Neighbor Distance/Density Criteria?}
    E -- Yes --> F[Classified as High-Confidence Region]
    E -- No --> G[Classified as Uncertain Region]
    F --> H[Increase Scan Interval]
    G --> I[Decrease Scan Interval]
    H --> J[Select Next Scan Point]
    I --> J
    J --> K[Update Measurements and Save to DB]
    K --> L{Termination Condition Met?}
    L -- No --> C
    L -- Yes --> M[End Scan and Generate Result Report]
```
